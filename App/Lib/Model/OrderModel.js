  ...
  return {
    ...
    update_orderState: Q.async(function*(oConfigJson){
      if(isObject(oConfigJson) && !isEmpty(oConfigJson)){
        ...
        yield D('Orderproductcopy').where(whereCaseJson).update({
     ...
     * }
     */
    get_orderMainInfo: Q.async(function*(selectId){
      if(isNumber(selectId)){
        return yield D('Orderproductcopy').where({id:selectId}).select();
      }
    }),
  }
})
